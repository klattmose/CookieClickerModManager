...
CCMM.LoadMod = function(id){
	...
	async function tabLoadMod(url) {
		const [tab] = await chrome.tabs.query({active: true, currentWindow: true});
		await chrome.scripting.executeScript({
			target: {tabId: tab.id},
			func: CCMM.ModInjection,
			args: [url]
		});
	}
...
CCMM.detectLoadedMods = function(){
	...
	function onCompletion(result){
		if(result === undefined || result['result'] === undefined) CCMM.LoadedMods = {};
		else CCMM.LoadedMods = result['result'];
		
		for(var i = 0; i < CCMM.config.mods.length; i++){
			var mod = CCMM.config.mods[i];
			if(CCMM.LoadedMods == -1) mod.isLoaded = 1;
			else if(CCMM.LoadedMods[CCMM.GuessModId(mod.url)]) mod.isLoaded = 1;
			else mod.isLoaded = 0;
		}
	...
	function whatScripts(){
		if(typeof CCMM == 'undefined') return -1;
